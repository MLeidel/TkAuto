...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        self.vlbl = StringVar()
        lbl = Label(self, text='LABEL TEXT', textvariable=self.vlbl)
        lbl.grid(row=1, column=1, sticky='ew')
        self.vlbl.set('LABEL TEXT')

        self.vent = StringVar()
        # self.vent.trace("w", self.eventHandler)
        ent = Entry(self, textvariable=self.vent)
        ent.grid(row=2, column=1, sticky='ew')

        btn = Button(self, text='Move To Label', command=self.btn_click)
        btn.grid(row=3, column=1, sticky='ew')

    def btn_click(self):
        ''' move text to label '''
        self.vlbl.set(self.vent.get())  # have to use StringVar for Label access
