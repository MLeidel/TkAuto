...
![draw](images/tkautoSketch.png "title")
...
### initial output
>This is a combination of a basic TkAuto template and the widgets
added in from the spreadsheet.
...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        lbl = Label(frame1, text='Type Something')
        lbl.grid(row=1, column=1)

        self.vent = StringVar()
        # self.vent.trace("w", self.eventHandler)
        ent = Entry(frame1, textvariable=self.vent)
        ent.grid(row=2, column=1)

        self.vchkbox = IntVar()
        chkbox = Checkbutton(self, variable=self.vchkbox, text='Over 18 years old')
        chkbox.grid(row=2, column=1)

        btn = Button(self, text='Close App', command=self.btn_close_app)
        btn.grid(row=3, column=1, sticky='ew')

    def btn_close_app(self):
        ''' docstring '''
...
app.mainloop()
```
### Final App after some editing
>After deleting the comments that are not needed and editing
some of the details in the code...
...
class Application(Frame):
    ...
    def create_widgets(self):
        ...
        # customize widget style when using ttk...
        style = Style()
        style.configure("TButton", width=15) # global
        ...
        self.vent = StringVar()
        ent = Entry(frame1, textvariable=self.vent)
        ent.grid(row=2, column=1, padx=8)
        ...
        btn = Button(self, text='Close App', command=self.btn_close_app)
        btn.grid(row=3, column=1, sticky='')

    def btn_close_app(self):
        ''' user closes this app '''
        print(self.vchkbox.get())
        print(self.vent.get())
        save_location()
